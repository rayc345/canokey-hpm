...
#define CONFIG_TOTAL_LEN (TUD_CONFIG_DESC_LEN + TUD_HID_INOUT_DESC_LEN + 9 + 0x36 + 14 + TUD_HID_DESC_LEN + 9)
//
...
uint8_t desc_fs_configuration[] =
    {
        ...
        // CCID Interface
        0x09, TUSB_DESC_INTERFACE, ITF_NUM_CCID, 0, 0x02, TUSB_CLASS_SMART_CARD, 0x00, 0x00, USBD_IDX_CCID_STR,
        /******************** Descriptor of CCID *************************/
...
uint16_t const *tud_descriptor_string_cb(uint8_t index, uint16_t langid)
{
  ...
  if (index < USBD_IDX_STD_STR_MAX)
  { // standard string, excluding sn
  ...
  if (index == USBD_IDX_SERIAL_STR)
  { // serial number
    uint8_t sn[4];
    static char sn_str[9];
    fill_sn(sn);
